...
class MainWindow(QWidget):
    ...
    def _update_teacher_table(self):
        ...
        for i, r in enumerate(records):
            ...
            self.cursor.execute("SELECT name FROM bin2001.subject WHERE name != %s", (str(r[2]),))
            temp_rec = list()
            temp_rec.append(r[2])
            temp_rec.extend(list(map(lambda x: str(x[0]), self.cursor.fetchall())))
            subject_records.extend(list(map(lambda x: str(x), temp_rec)))
            teacher_records = list()
            teacher_records.extend(list(map(lambda x: str(x[1]), records)))
            subjects = list()
            teachers = list()
            #try:
    ...
    def _add_new_subject(self, subject):
        try:
            ...
            #self.cursor.execute(
            #    "INSERT INTO bin2001.teacher(subject, full_name) VALUES (%s, %s),", (str(subject), None)
            #)
            self.conn.commit()
